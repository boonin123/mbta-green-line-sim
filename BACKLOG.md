...
| Pri | Item | Notes |
|-----|------|-------|
| P1 | **Investigate outer-branch boarding saturation** | Trains appear full for too long past stations like Coolidge Corner (C branch, outbound). Likely causes: (a) `ALIGHT_FRACTIONS["branch_outer"] = 0.10` may be too low — outer stops see lots of exits, not just boardings; (b) `pax_scale` not differentiating between trunk-level and branch-only station rates; (c) outbound direction accumulates passengers at terminus but doesn't model reverse-peak commuters well. Validate against MBTA ridership data (Blue Book / CTPS). |
| P1 | **Short-turn trains** | Trains that terminate early (e.g. a B branch train turning at Kenmore rather than Boston College) are a documented Green Line delay source. Currently not modelled — all trains run the full configured route. Add `short_turn_prob` to SimConfig and dispatcher logic. |
